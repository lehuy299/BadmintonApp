...
public class GetAvalableSlotHandler extends Handler {
    ...
    protected ResponseEntity<GetAvalableSlotResponse> DoPost(InputStream InputStream) {
        ...
        GetAllCenterInCityRequest GetAllCenterInCityRequest = new GetAllCenterInCityRequest();
        GetAllCenterInCityRequest.CityID = GetAvalableSlotRequest.CityID;

        ArrayList<FreeSlot> AllFreeSlots = new ArrayList<>();
        ArrayList<Center> CenterList = GetAllCenterInCitySql.Get(GetAllCenterInCityRequest).Centers;
        ...
        if (CenterList.size() > 0) {
            for (int i = 0; i < CenterList.size(); i++) {
                ...
                GetAllCourtInCenterRequest.CityID = GetAvalableSlotRequest.CityID;
                GetAllCourtInCenterRequest.CenterID = CurrentCenter.CenterID;

                ArrayList<Court> CourtList = GetAllCourtInCenterSql.Get(GetAllCourtInCenterRequest).Courts;
